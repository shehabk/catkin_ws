...
class RandomWalk {
	...
	void commandCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
		if (fsm == FSM_MOVE_FORWARD) {
			...
			if(closestRange < PROXIMITY_RANGE_M){
				...
				rotateStartTime = ros::Time::now();
				if(rand()%2==0){
					// rotateDuration = ros::Duration( ROTATE_SPEED_FACTOR*(ROTATE_NINETY_TIME + (rand()%50 / 100.0) ));
					rotateDuration = ros::Duration( ROTATE_NINETY_TIME*(2 + ((rand()%60) / 100.0) ));
					//rotateDuration = ros::Duration( 2*ROTATE_NINETY_TIME+(rand()%200)/100);
				}else{
					//rotateDuration = ros::Duration( ROTATE_SPEED_FACTOR*(ROTATE_NINETY_TIME - (rand()%50 / 100.0) )); 
					rotateDuration = ros::Duration( ROTATE_NINETY_TIME*(2 - ((rand()%60) / 100.0) ));
				}
	...
	void spin() {
		ros::Rate rate(500); // Specify the FSM loop rate in Hz
	...
	const static double FORWARD_SPEED_MPS = 0.2;//1.0;
	const static double ROTATE_SPEED_RADPS = M_PI/4; //M_PI/2;
	const static double ROTATE_NINETY_TIME 	= 2 ;  		 
