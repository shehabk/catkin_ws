...
class RandomWalk {
  ...
  // TODO: if range is smaller than PROXIMITY_RANGE_M, update fsmand rotateStartTime,
  ...
  /////////////////////// ANSWER CODE BEGIN ///////////////////
    ...
    if(closestRange < PROXIMITY_RANGE_M)
    {
       fsm = FSM_ROTATE;
       rotateStartTime = ros::Time::now();
       rotateDuration = ros::Duration(rand() % 10);
    }
  ...
  void spin() {
    ros::Rate rate(30); // Specify the FSM looprate in Hz
